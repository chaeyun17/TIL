...
반대로 상속방식인 `CGLIB` 방식으로 사용할 때는 프록시빈 대상 클래스에 상속하지 못하는 접근제한자 private, default, final 키워드드 걸린 필드 및 메서드들이 프록시 클래스를 생성할 때 접근하지 못해서 동작하지 못한다. bean 생성 시 에러 또는 동작 시 동작하지 않으니 조심해야한다.
